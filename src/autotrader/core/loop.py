...
class TradingLoop:
    ...
    async def _tick_inner(self) -> None:
        ...
        #    Also collects mispricing proposals from strategies.
        market_proposals = await self._refresh_market_data()
        ...
        if new_snapshot is not None and new_snapshot is not prev_snapshot:
            ...
            # Seed strategy rankings from the first successful snapshot so
            # mispricing detection has fair-value estimates immediately.
            if not self._rankings_seeded:
                self._seed_strategy_rankings(new_snapshot)

        # 2. Feed signals to all strategies → collect signal-driven proposals
        signal_proposals: list[ProposedOrder] = []
        if signals:
            ...
            for signal in signals:
                for strat in self._strategies.values():
                    ...
                    signal_proposals.extend(proposals)

        # 3. Combine signal-driven and market-driven proposals
        all_proposals: list[ProposedOrder] = signal_proposals + market_proposals
    ...
    async def _refresh_market_data(self) -> list[ProposedOrder]:
        ...
        Returns any trade proposals generated by strategies during market
        data processing (e.g. mispricing detection).
        """
    ...
    def _seed_strategy_rankings(self, snapshot: LeaderboardSnapshot) -> None:
        """Seed strategy rankings from the first successful Arena snapshot.
