...
class LeaderboardAlphaStrategy(Strategy):
    ...
    def _resolve_ticker(self, model_name: str, series: str | None = None) -> str | None:
        ...
        if series == "KXLLM1":
            override_ticker = (
                self._config.org_ticker_overrides.get(model_name)
                or self._config.org_ticker_overrides.get(resolved_name)
            )
        else:
            override_ticker = (
                self._config.model_ticker_overrides.get(model_name)
                or self._config.model_ticker_overrides.get(resolved_name)
            )

        if override_ticker:
            if override_ticker in self._ticker_model_names and (series is None or self._ticker_series(override_ticker) == series):
                cache = self._org_ticker_map if self._ticker_series(override_ticker) == "KXLLM1" else self._model_ticker_map
